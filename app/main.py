...
async def decode_qr(file: UploadFile = File(...)):
    ...
    try:
        ...
        for i, data in enumerate(qr_data[1:]):  # Procesamos desde el segundo elemento
            if re.match(r'^\d{2}$', data):  # Tipo de Documento
                tipo_doc_map = {
            ...
            elif re.match(r'^[A-Za-z0-9]{2,4}$', data):  # Serie (B255)
                result["serie"] = data
                print(f"Serie detectada: {data}")
            elif re.match(r'^\d{7,8}$', data):  # Número (00047683)
                result["numero"] = data.zfill(8)
                print(f"Número detectado: {data.zfill(8)}")
            elif re.match(r'^\d{8}$', data) and 'numero' in result:  # DNI (66666666)
                result["dni"] = data
                print(f"DNI detectado: {data}")
            elif re.match(r'^\d+\.?\d{0,2}$', data):  # Valor monetario (0.92 o 6)
                # Acepta números con o sin decimales
        ...
        if monetary_values:
            # Convertir a float para ordenar correctamente
            monetary_values_float = [float(x) for x in monetary_values]
            monetary_values_sorted = sorted(monetary_values_float, reverse=True)
            
            # Convertir de vuelta a string manteniendo el formato original (6 vs 6.00)
            monetary_values_str = []
            for val, orig in zip(monetary_values_sorted, monetary_values):
                if '.' in orig:
                    monetary_values_str.append(f"{val:.2f}")
                else:
                    monetary_values_str.append(f"{int(val)}")
            
            result["total"] = monetary_values_str[0]
            
            if len(monetary_values_str) > 1:
                result["igv"] = monetary_values_str[1]
            
            if len(monetary_values_str) > 2:
                result["sub_total"] = monetary_values_str[2]
    ...
    try:
        ...
        for i, data in enumerate(qr_data[1:]):  # Procesamos desde el segundo elemento
            if re.match(r'^\d{2}$', data):  # Tipo de Documento
                ...
                result["tipo"] = tipo_doc_map.get(data, "Desconocido")
                print(f"Tipo de documento detectado: {result['tipo']} ({data})")
            elif re.match(r'^[A-Za-z0-9]{2,4}$', data):  # Serie (B31Y)
                result["serie"] = data
                print(f"Serie detectada: {data}")
            elif re.match(r'^\d{7,8}$', data):  # Número (00505070)
                result["numero"] = data.zfill(8)
                print(f"Número detectado: {data.zfill(8)}")
            elif re.match(r'^\d+\.\d{1,2}$', data):  # Valor monetario
                monetary_values.append(data)
                print(f"Valor monetario detectado: {data}")
            elif re.match(r'^\d{4}-\d{2}-\d{2}$', data) or re.match(r'^\d{2}/\d{2}/\d{4}$', data):  # Fecha
                result["fecha"] = data
